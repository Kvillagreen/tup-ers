...
if (!$email || !$hostEmail || !$hostPassword || !$password) {
  echo json_encode([
    'success' => false,
...
if ($result->num_rows > 0) {
  ...
  $recepientEmail = $faculty['email'];
  $recepientName = $faculty['first_name'] . ' ' . $faculty['last_name'];
  $updateSql = "UPDATE tbl_admin SET password = ?, otp_code = ? WHERE email = ?";
  $updateStmt = $conn->prepare($updateSql);
  $updateStmt->bind_param("sss", $passwordHashed, $otpCode, $email);
  $updateStmt->execute();
} else {
